...
public final class ContextBinder {
     ...
     * 在作用域内执行，有返回值
     */
    public static <T> T callWithContext(VisitorContext visitor, UserContext user,
                                        TenantContext tenant, I18nContext i18n, Callable<T> op)
            throws Exception {
     ...
     * 在作用域内执行，无返回值
     */
    public static void runWithContext(VisitorContext visitor, UserContext user,
                                      TenantContext tenant, I18nContext i18n, Runnable op) {
        ScopedValue.Carrier carrier = carrierOf(visitor, user, tenant, i18n);
        carrier.run(op);
    }
