...
import cc.uncarbon.module.file.service.OssFileInfoService;
...
public class FileUpDownloadFacadeImpl implements FileUpDownloadFacade {
    ...
    public @NonNull FileDownloadReply downloadById(@Nullable String tenantCode, Long fileMetaId) {
        ...
        // 不要直接返回前端租户不存在，隐藏技术细节
        FileErrorCodeEnum.A02006.throwIfNull(tenantMeta);
        ...
        byte[] fileBytes = null;
        if (
                OssFileInfoService.isLocalPlatform(fileMeta.getStoragePlatform())
                        || CharSequenceUtil.isEmpty(fileMeta.getDirectUrl())
        ) {
            FileInfo fileInfo = OssFileInfoService.toFileInfo(fileMeta);
            try {
                fileBytes = fileStorageService.download(fileInfo).bytes();
            } catch (FileStorageRuntimeException fsre) {
                log.error(LOG_PREFIX + "[下载] FileStorageRuntimeException >> ", fsre);
                throw new BusinessException(FileErrorCodeEnum.B02002);
            }
        } else {
