...
public class AdminSysDictController {
    ...
    public ApiResult<PageResult<SysDataDictClassifiedBO>> list(PageParam pageParam, AdminSysDataDictClassifiedListDTO dto) {
        return ApiResult.success(sysDataDictService.adminListClassified(pageParam, dto));
    }

    @SysOperateLog(value = "新增数据字典分类")
    ...
    public ApiResult<PageResult<SysDataDictItemBO>> list(@PathVariable Long classifiedId, PageParam pageParam, AdminSysDataDictItemListDTO dto) {
        ...
        return ApiResult.success(sysDataDictService.adminListItem(pageParam, dto));
    }

    @SysOperateLog(value = "新增数据字典项")
    @SaCheckPermission(type = StpLoginType.ADMIN, value = PERMISSION_PREFIX + PermissionPattern.CREATE)
